...
static int load_local_ble_identities(void)
{
#if defined(CONFIG_BT_SETTINGS)
    int err = settings_load_subtree("bt");
    if (err && err != -ENOENT) {
...
int transport_start()
{
    ...
    err = load_local_ble_identities();
    if (err) {
        LOG_WRN("Continuing without loaded BT identities (err %d)", err);
    }
